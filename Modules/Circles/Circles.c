...
#define SAVERWINDOW_CLASS "CIRCLES_SCREENSAVER_CLASS"
...
static FILE *internal_OpenNLSFile(char *pchHomeDirectory)
{
  ...
  if (pchFileName)
  {
	...
	sprintf(pchFileName, "%sModules\\Circles\\%s.msg", pchHomeDirectory, achRealLocaleName);
#ifdef DEBUG_LOGGING
	...
	do {
	  // Assemble NLS file name
	  sprintf(pchFileName, "%sModules\\Circles\\%s.msg", pchHomeDirectory, pchLang);
#ifdef DEBUG_LOGGING
	  AddLog("[internal_OpenNLSFile] : Trying to open NLS file: [");
	  AddLog(pchFileName);
...
static void internal_LoadConfig(ModuleConfig_p pConfig, char *pchHomeDirectory)
{
  ...
  if (pchHomeEnvVar)
  {
    ...
    strncat(achFileName, ".dssaver\\Circles.CFG", sizeof(achFileName));
  ...
  if (!hFile)
  {
    ...
    snprintf(achFileName, sizeof(achFileName), "%sCircles.CFG", pchHomeDirectory);
    hFile = fopen(achFileName, "rt");
    if (!hFile)
      return; // Could not open file!
  }

  iScanned = fscanf(hFile, "%d %d %d %d %d %d",
                    &(TempConfig.iResType),
  ...
  if (iScanned == 6)
  {
    ...
    AddLog("[internal_LoadConfig] : Could not parse all the 6 settings, using defaults!\n");
#endif
...
static void internal_SaveConfig(ModuleConfig_p pConfig, char *pchHomeDirectory)
{
  ...
  if (pchHomeEnvVar)
  {
    ...
    strncat(achFileName, "\\Circles.CFG", sizeof(achFileName));
    // Try to open that config file!
...
static void internal_InvertEnds(char *pchBuffer, int iScanLineSize, int x1, int y1, int x2, int c1, int c2)
{
  int iBytesPerPixel;
  ...
  if (x1<0) x1 = 0;
  if (x1>=iBufferXSize) x1 = iBufferXSize-1;
  if (x2<0) x2 = 0;
  if (x2>=iBufferXSize) x2 = iBufferXSize-1;
  ...
  if ((y1>=0) && (y1<iBufferYSize))
  {
    if (pchBuffer[y1*iScanLineSize + x1*iBytesPerPixel] == c1)
      pchBuffer[y1*iScanLineSize + x1*iBytesPerPixel] = c2;
    else
      pchBuffer[y1*iScanLineSize + x1*iBytesPerPixel] = c1;
    if (pchBuffer[y1*iScanLineSize + x2*iBytesPerPixel] == c1)
      pchBuffer[y1*iScanLineSize + x2*iBytesPerPixel] = c2;
    else
      pchBuffer[y1*iScanLineSize + x2*iBytesPerPixel] = c1;
  }
}

static void internal_Circle(char *pchBuffer, int iScanLineSize, int sx, int sy, int r, int c1, int c2)
{
  int x = 0;
  int y = r;
  int d = 1 - r;
  ...
  internal_InvertEnds(pchBuffer, iScanLineSize, sx - y, sy + x, sx + y, c1, c2);
  ...
  while (x <= y)
  {
    ...
    } else
    {
      ...
      if (x<=y)
      {
        internal_InvertEnds(pchBuffer, iScanLineSize, sx - x, sy + y, sx + x, c1, c2);
        internal_InvertEnds(pchBuffer, iScanLineSize, sx - x, sy - y, sx + x, c1, c2);
        if (x!=y)
        {
          internal_InvertEnds(pchBuffer, iScanLineSize, sx - y, sy + x, sx + y, c1, c2);
...
#define MAX_NUM_OF_CENTERS 4

void fnBlitThread(void *p)
{
  int iNumOfCenters;
  int iCenterX[MAX_NUM_OF_CENTERS];
  int iCenterY[MAX_NUM_OF_CENTERS];
  int iCenterSpeedX[MAX_NUM_OF_CENTERS];
  int iCenterSpeedY[MAX_NUM_OF_CENTERS];
  int iMaxXSpeed, iMaxYSpeed;
  int iColor1Index;
  int iColor2Index;
  int i, bFlag;
  int x, y;
  ...
  iMaxXSpeed = iBufferXSize / 100;
  iMaxYSpeed = iBufferYSize / 100;

  iNumOfCenters = rand()*MAX_NUM_OF_CENTERS;
  iNumOfCenters /= RAND_MAX;
  if (iNumOfCenters<=1) iNumOfCenters = 2;

  for (i=0; i<iNumOfCenters; i++)
  {
    iCenterX[i] = rand() * iBufferXSize;
    iCenterX[i] /= RAND_MAX;
    iCenterY[i] = rand() * iBufferYSize;
    iCenterY[i] /= RAND_MAX;
    iCenterSpeedX[i] = rand() * iMaxXSpeed*2;
    iCenterSpeedX[i] /= RAND_MAX;
    iCenterSpeedX[i] -= iMaxXSpeed;
    if (iCenterSpeedX[i]==0)
      iCenterSpeedX[i] = 1;
    iCenterSpeedY[i] = rand() * iMaxYSpeed*2;
    iCenterSpeedY[i] /= RAND_MAX;
    iCenterSpeedY[i] -= iMaxYSpeed;
    if (iCenterSpeedY[i]==0)
      iCenterSpeedY[i] = 1;

  }
  iColor1Index = rand() * 256;
  iColor1Index/=RAND_MAX;
  do {
    iColor2Index = rand() * 256;
    iColor2Index/=RAND_MAX;
  } while (iColor2Index == iColor1Index);
  ...
  if (bHWVideoPresent)
  {
    ...
    vidmem = (PBYTE) malloc(ulScanLineBytes * ulScanLines);
  ...
  while (!bShutdownRequest)
  {
	...
	i = 0;
      ...
      if (!i)
      {
        ...
        // Move centers and change speed and direction if
        // reached sides
        for (i=0; i<iNumOfCenters; i++)
        {
          iCenterX[i] += iCenterSpeedX[i];
          iCenterY[i] += iCenterSpeedY[i];
          if (iCenterX[i]<=0)
          {
            iCenterX[i] = 0;
            iCenterSpeedX[i] = rand() * iMaxXSpeed;
            iCenterSpeedX[i] /= RAND_MAX;
            if (iCenterSpeedX[i]==0)
              iCenterSpeedX[i] = 1;
          }
          ...
          {
            iCenterY[i] = 0;
            iCenterSpeedY[i] = rand() * iMaxXSpeed;
            iCenterSpeedY[i] /= RAND_MAX;
            if (iCenterSpeedY[i]==0)
              iCenterSpeedY[i] = 1;
          }
          if (iCenterX[i]>=iBufferXSize)
          {
            iCenterX[i] = iBufferXSize;
            iCenterSpeedX[i] = rand() * iMaxXSpeed;
            iCenterSpeedX[i] /= RAND_MAX;
            if (iCenterSpeedX[i]==0)
              iCenterSpeedX[i] = 1;
            iCenterSpeedX[i] = -iCenterSpeedX[i];
          }
          if (iCenterY[i]>=iBufferYSize)
          {
            iCenterY[i] = iBufferYSize;
            iCenterSpeedY[i] = rand() * iMaxXSpeed;
            iCenterSpeedY[i] /= RAND_MAX;
            if (iCenterSpeedY[i]==0)
              iCenterSpeedY[i] = 1;
            iCenterSpeedY[i] = -iCenterSpeedY[i];
          }
        }

        // Set base color
        memset(vidmem, iColor1Index, ulScanLineBytes * ulScanLines);
        ...
        for (i=0; i<iNumOfCenters; i++)
        {
          ...
          maxr = (int) sqrt(iBufferXSize*iBufferXSize + iBufferYSize*iBufferYSize);
          
          r = 25;

          while (r<=maxr)
          {
            internal_Circle(vidmem, ulScanLineBytes,
                            iCenterX[i], iCenterY[i], r,
                            iColor1Index, iColor2Index);
            r+=25;
          }
        }

        // Fill colors!
        for (y=0; y<ulScanLines; y++)
        {
          // Now go through this line, and set pixels!
          bFlag = 0;
          pchTemp = vidmem + y*ulScanLineBytes;
          for (x=0; x<iBufferXSize; x++)
          {
            if (!bHWVideoPresent)
            {
              // Palettized mode
              if ((*pchTemp) == iColor2Index)
                bFlag = !bFlag;

              if (bFlag)
                (*pchTemp) = iColor2Index;
            ...
            } else
            {
              ...
              if (bFlag)
              {
                (*pchTemp) = yuvpal_y[iColor2Index];
                if (x%2==0)
                  (*(pchTemp+1)) = yuvpal_u[iColor2Index];
                else
                  (*(pchTemp+1)) = yuvpal_v[iColor2Index];
              } else
              {
                (*pchTemp) = yuvpal_y[iColor1Index];
                if (x%2==0)
                  (*(pchTemp+1)) = yuvpal_u[iColor1Index];
                else
                  (*(pchTemp+1)) = yuvpal_v[iColor1Index];
              }
              pchTemp+=2;
            }
          }
...
//            AddLog("[fnBlitThread] : Time! Blitting.\n");
            ...
            if (DosRequestMutexSem(hmtxUseDive, SEM_INDEFINITE_WAIT)==NO_ERROR)
            {
              ...
              else
              {
                ...
                {
                  memcpy(pchWork, vidmem, ulScanLineBytes * ulScanLines);
                  pfnHWVIDEOEndUpdate();
                }
              }
              DosReleaseMutexSem(hmtxUseDive);
