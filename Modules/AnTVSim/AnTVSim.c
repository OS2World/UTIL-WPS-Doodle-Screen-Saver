...
// Module config:
// Fading seems to cause problems, the logic might be faulty...
// For now, it's simply disabled. :)
//#define USE_FADING
...
#define SAVERWINDOW_CLASS "ANTVSIM_SCREENSAVER_CLASS"
...
#ifdef USE_FADING
#define WM_FADEIN_DONE         (WM_USER+7)
#define WM_FADEOUT_DONE        (WM_USER+8)
#endif
...
int      bFadeIn, bFadeOut, bFinalFadeOut;
...
static FILE *internal_OpenNLSFile(char *pchHomeDirectory)
{
  ...
  if (pchFileName)
  {
	...
	sprintf(pchFileName, "%sModules\\AnTVSim\\%s.msg", pchHomeDirectory, achRealLocaleName);
#ifdef DEBUG_LOGGING
	...
	do {
	  // Assemble NLS file name
	  sprintf(pchFileName, "%sModules\\AnTVSim\\%s.msg", pchHomeDirectory, pchLang);
#ifdef DEBUG_LOGGING
	  AddLog("[internal_OpenNLSFile] : Trying to open NLS file: [");
	  AddLog(pchFileName);
...
static void internal_LoadConfig(ModuleConfig_p pConfig, char *pchHomeDirectory)
{
  ...
  if (pchHomeEnvVar)
  {
    ...
    strncat(achFileName, ".dssaver\\AnTVSim.CFG", sizeof(achFileName));
  ...
  if (!hFile)
  {
    ...
    snprintf(achFileName, sizeof(achFileName), "%sAnTVSim.CFG", pchHomeDirectory);
    hFile = fopen(achFileName, "rt");
    if (!hFile)
      return; // Could not open file!
  }

  iScanned = fscanf(hFile, "%d %d %d %d %d",
                    &(TempConfig.iResType),
  ...
  if (iScanned == 5)
  {
    ...
    AddLog("[internal_LoadConfig] : Could not parse all the 5 settings, using defaults!\n");
#endif
...
static void internal_SaveConfig(ModuleConfig_p pConfig, char *pchHomeDirectory)
{
  ...
  if (pchHomeEnvVar)
  {
    ...
    strncat(achFileName, "\\AnTVSim.CFG", sizeof(achFileName));
    // Try to open that config file!
...
void fnBlitterThread(void *p)
{
  ...
  int iRandomColor;
  int iX, iY, iR, iG, iB, iCount;
  char *pchSrc;
  char *pchDst;
  int iTemp;
  int iShiftStartPos, iShiftSize;
  int iShiftForThisLine;
  int iSmallNoiseStartPos1, iSmallNoiseStartPos2;
  int iSmallNoiseSize1, iSmallNoiseSize2;
  int bAddNoise = 0;
  int bAddSmallNoise1 = 0;
  int bAddSmallNoise2 = 0;
  int bAddShiftedImage = 0;
  int bMakeImagePale = 0;
  int iFadeDirection;

  int iFadeVisibilityPercentage;
  ...
  srand(time(NULL));
  for (iRand=0; iRand<RAND_BUF_SIZE; iRand++)
    aiRandArray[iRand] = rand()*256/RAND_MAX;
  iRand = 0;

  // Initialize variables
  if (bOnlyPreviewMode)
    iFadeVisibilityPercentage = 100;
  else
    iFadeVisibilityPercentage = 0; // Start with full black if not in preview mode!

  iShiftStartPos = rand() * iBufferYSize / RAND_MAX;
  iShiftSize = rand() * iBufferYSize/3 /RAND_MAX;

  iSmallNoiseStartPos1 = rand() * iBufferYSize / RAND_MAX;
  iSmallNoiseSize1 = rand() * 50 /RAND_MAX;

  iSmallNoiseStartPos2 = rand() * iBufferYSize / RAND_MAX;
  iSmallNoiseSize2 = rand() * 50 /RAND_MAX;
...
//        AddLog("[fnBlitterThread] : Getting pchWork from HWVIDEO\n");
	...
	iTemp = pfnHWVIDEOBeginUpdate(&pchWork, &ulPhysAddr);
        //ulWScanLineBytes = iBufferXSize * (iBufferBPP+7)/8;
...
//	AddLog("[fnBlitterThread] : Got pchWork\n");
	...
	if (iFadeDirection>0)
	{
	  if (iFadeVisibilityPercentage>(iFadeDirection==2?50:0))
            iFadeVisibilityPercentage-=5;
#ifdef USE_FADING
	  else
	  {
#ifdef DEBUG_LOGGING
	    AddLog("[fnBlitterThread] : FADEOUT_DONE\n");
#endif
	    iFadeDirection = 0;
	    WinPostQueueMsg(hmqSaverThreadMsgQueue, WM_FADEOUT_DONE, (MPARAM) 0, (MPARAM) 0);
	  }
#endif
        }
	if (iFadeDirection<0)
	{
	  if (iFadeVisibilityPercentage<100)
            iFadeVisibilityPercentage+=5;
#ifdef USE_FADING
	  else
	  {
#ifdef DEBUG_LOGGING
	    AddLog("[fnBlitterThread] : FADEIN_DONE\n");
#endif
	    iFadeDirection = 0;
	    WinPostQueueMsg(hmqSaverThreadMsgQueue, WM_FADEIN_DONE, (MPARAM) 0, (MPARAM) 0);
          }
#endif
	}
	...
	if (bFadeIn)
	{
	  ...
	  {
#ifdef DEBUG_LOGGING
	    AddLog("[fnBlitterThread] : Fading in!\n");
#endif
#ifdef USE_FADING
	    if (iFadeDirection>0)
	    {
	      // Was fading out, so stop fadeout
#ifdef DEBUG_LOGGING
	      AddLog("[fnBlitterThread] : FADEOUT_DONE\n");
#endif
	      WinPostQueueMsg(hmqSaverThreadMsgQueue, WM_FADEOUT_DONE, (MPARAM) 0, (MPARAM) 0);
	...
	{
#ifdef DEBUG_LOGGING
	  AddLog("[fnBlitterThread] : Fading Out!\n");
#endif
#ifdef USE_FADING
	  if (iFadeDirection<0)
	  {
	    // Was fading in, so stop fadein!
#ifdef DEBUG_LOGGING
	    AddLog("[fnBlitterThread] : FADEIN_DONE\n");
#endif
	    WinPostQueueMsg(hmqSaverThreadMsgQueue, WM_FADEIN_DONE, (MPARAM) 0, (MPARAM) 0);
...
#endif
	  iFadeDirection = bFadeOut;
	  bFadeOut = 0;
	}

	if (iFadeVisibilityPercentage<0)
	  iFadeVisibilityPercentage = 0;
	...
	if (bAddNoise)
	{
	  // Noise is on, turn it off quickly
	  if (rand() * 1000 / RAND_MAX < 100)
	    bAddNoise = FALSE;
	} else
	{
	  // Noise is off, turn it on sometimes!
	  if (rand() * 1000 / RAND_MAX < 15)
	    bAddNoise = TRUE;
	}

	if (bAddSmallNoise1)
	{
	  // SmallNoise is on, turn it off quickly
	  if (rand() * 1000 / RAND_MAX < 30)
	    bAddSmallNoise1 = FALSE;
	} else
	{
	  // SmallNoise is off, turn it on sometimes!
	  if (rand() * 1000 / RAND_MAX < 120)
	    bAddSmallNoise1 = TRUE;
	}

	if (bAddSmallNoise2)
	{
	  // SmallNoise is on, turn it off quickly
	  if (rand() * 1000 / RAND_MAX < 50)
	    bAddSmallNoise2 = FALSE;
	} else
	{
	  // SmallNoise is off, turn it on sometimes!
	  if (rand() * 1000 / RAND_MAX < 100)
	    bAddSmallNoise2 = TRUE;
	}

	if (bAddShiftedImage)
	{
	  // ShiftedImage is on, turn it off quickly
	  if (rand() * 1000 / RAND_MAX < 100)
	    bAddShiftedImage = FALSE;
	} else
	{
	  // ShiftedImage is off, turn it on sometimes!
	  if (rand() * 1000 / RAND_MAX < 80)
	    bAddShiftedImage = TRUE;
	}

	bMakeImagePale = (rand() * 1000 / RAND_MAX < 900);

	// Per-frame initialization of shifted image stuffs
	iTemp = rand() * 15 / RAND_MAX;
	if (rand()*10/RAND_MAX < 5)
	  iTemp = -iTemp;
	iShiftStartPos += iTemp;

	if ((iShiftStartPos < 0) ||
	    (iShiftStartPos>iBufferYSize))
	{
	  iShiftStartPos = rand() * iBufferYSize / RAND_MAX;
	}
	...
	if ((iShiftSize < 0) ||
	    (iShiftSize>iBufferXSize/2))
	{
	  iShiftSize = rand() * iBufferXSize/3 / RAND_MAX;
	}

	// Per-frame initialization of small noises
	iTemp = rand() * 15 / RAND_MAX;
	if (rand()*10/RAND_MAX < 5)
	  iTemp = -iTemp;
	iSmallNoiseStartPos1 += iTemp;

	if ((iSmallNoiseStartPos1 < 0) ||
	    (iSmallNoiseStartPos1>iBufferYSize) ||
	    (rand()*1000/RAND_MAX==0))
	{
	  iSmallNoiseStartPos1 = rand() * iBufferYSize / RAND_MAX;
	}

	iTemp = rand() * 5 / RAND_MAX;
	if (rand()*10/RAND_MAX < 5)
	  iTemp = -iTemp;
	iSmallNoiseSize1 += iTemp;

	if ((iSmallNoiseSize1 < 0) ||
	    (iSmallNoiseSize1>iBufferXSize/5))
	{
	  iSmallNoiseSize1 = rand() * iBufferXSize/50 / RAND_MAX;
	}

	iTemp = rand() * 15 / RAND_MAX;
	if (rand()*10/RAND_MAX < 5)
	  iTemp = -iTemp;
	iSmallNoiseStartPos2 += iTemp;

	if ((iSmallNoiseStartPos2 < 0) ||
	    (iSmallNoiseStartPos2>iBufferYSize) ||
	    (rand()*1000/RAND_MAX==0))
	{
	  iSmallNoiseStartPos2 = rand() * iBufferYSize / RAND_MAX;
	}
	...
	if (rand()*10/RAND_MAX < 5)
	  iTemp = -iTemp;
	iSmallNoiseSize2 += iTemp;

	if ((iSmallNoiseSize2 < 0) ||
	    (iSmallNoiseSize2>iBufferXSize/5))
	{
	  iSmallNoiseSize2 = rand() * iBufferXSize/50 / RAND_MAX;
	}

	if (!bPaused)
	{
#ifdef DEBUG_LOGGING
//	  AddLog("[fnBlitterThread] : Per-line calculation begin\n");
//	  {
//	    char achTemp[256];
//	    sprintf(achTemp, "%p", pchScreenShot);
//	    AddLog("pchScreenShot = 0x");AddLog(achTemp);AddLog("\n");
//            sprintf(achTemp, "%p", pchWork);
//            AddLog("pchWork       = 0x");AddLog(achTemp);AddLog("\n");
//	  }
#endif

	  // Do for every line:
	  for (iY = 0; iY<iBufferYSize; iY++)
	  {
	    int iPerLineGrayness;

	    pchSrc = pchScreenShot + iY * ulSSScanLineBytes;
	    pchDst = pchWork + iY * ulWScanLineBytes;

	    iPerLineGrayness = 20 + (rand()*70)/RAND_MAX;

	    iShiftForThisLine = (iBufferBPP+7)/8* (int) ((iShiftSize - (iY-iShiftStartPos)) * sin(3.1415/180*(iShiftStartPos-iY)*(iBufferBPP+7)/8));

//	    AddLog("[fnBlitterThread] : One line\n");

	    for (iX = 0; iX<iBufferXSize; iX++)
	    {
	      iB = *(pchSrc++);  // or Y
	      iG = *(pchSrc++);  // or U or V
		...
		iR = *(pchSrc++);
	      ...
	      // Per pixel stuffs:
	      // Make image less colorful (per-line grayness)
	      if (bMakeImagePale)
	      {
		int iNewR, iNewG, iNewB;
		int iRem = (100-iPerLineGrayness)/2;

		if (!bHWVideoPresent)
		{
		  // Make RGB more pale
		  iNewR = ((iR * iPerLineGrayness) + (iG * iRem) + (iB*iRem)) / 100;
		  iNewG = ((iR * iRem) + (iG * iPerLineGrayness) + (iB*iRem)) / 100;
		  iNewB = ((iR * iRem) + (iG * iRem) + (iB*iPerLineGrayness)) / 100;
		  iR = iNewR;
		  iG = iNewG;
		  iB = iNewB;
		} else
		{
		  // Make YUV more pale
		  iG = 0x80 + (iG-0x80) * iPerLineGrayness / 100; // U or V
		}
	      }
	      ...
	      if (bAddNoise)
	      {
		iRandomColor = aiRandArray[iRand];
		iRand = (iRand+1) % RAND_BUF_SIZE;
		...
		{
		  iG = iCount * 0x80 + (iG-iCount*0x80) * 80 / 100;
		  iB = iB * 80 / 100;
		  iG += 0x80;
		  iB += iRandomColor;
		}
	      ...
	      // Add small noise lines
	      if (bAddSmallNoise1)
	      {
		if ((iSmallNoiseStartPos1>=iY) && (iSmallNoiseStartPos1<=iY+iSmallNoiseSize1))
		{
		  iRandomColor = aiRandArray[iRand];
		  iRand = (iRand+1) % RAND_BUF_SIZE;
		  if (!bHWVideoPresent)
	      ...
	      if (bAddSmallNoise2)
	      {
		if ((iSmallNoiseStartPos2>=iY) && (iSmallNoiseStartPos2<=iY+iSmallNoiseSize2))
		{
		  iRandomColor = aiRandArray[iRand];
		  iRand = (iRand+1) % RAND_BUF_SIZE;
		  if (!bHWVideoPresent)
		...
		iDiff = iShiftForThisLine;

		if (!bHWVideoPresent)
		{
                  // RGB
		  if (pchSrc-3+iDiff < pchScreenShot)
		    iDiff = 0;
		  if (pchSrc+iDiff > pchScreenShot + ulSSScanLineBytes * ulSSScanLines)
		    iDiff = 0;

		  if ((iX*2 + iDiff < 0) || (iX*2 + iDiff >= iBufferXSize*2))
		  {
                    iRandomColor = aiRandArray[iRand];
		    iRand = (iRand+1) % RAND_BUF_SIZE;
                    iR += iRandomColor;
		...
		{
		  // YUV
		  if (pchSrc-2+iDiff < pchScreenShot)
		    iDiff = 0;
		  if (pchSrc+iDiff > pchScreenShot + ulSSScanLineBytes * ulSSScanLines)
		    iDiff = 0;
		...
		{
                  // RGB
		  iR = iR*iFadeVisibilityPercentage/100;
		  iG = iG*iFadeVisibilityPercentage/100;
		  iB = iB*iFadeVisibilityPercentage/100;
		} else
		{
		  // YUV
		  iG = iCount * 0x80 + (iG-iCount*0x80) * iPerLineGrayness / 100; // U or V
		  iB = iB*iFadeVisibilityPercentage/100;
		}
	      }
		...
		*(pchDst++) = iB;
            ...
            } // End of loop of X direction
	  } // End of loop of Y direction
	} // End of if (!bPaused)
...
#ifdef DEBUG_LOGGING
        AddLog("[fnSaverThread] : Waiting for WM_FADEIN_DONE\n");
#endif

#ifdef USE_FADING
        // Now process until we get WM_FADEIN_DONE!
        while (WinGetMsg(hab, &msg, 0, 0, 0) && (msg.msg!=WM_FADEIN_DONE))
          WinDispatchMsg(hab, &msg);

#ifdef DEBUG_LOGGING
        AddLog("[fnSaverThread] : WM_FADEIN_DONE is here, starting real saving!\n");
#endif
#endif
...
#ifdef USE_FADING
#ifdef DEBUG_LOGGING
        AddLog("[fnSaverThread] : Waiting for WM_FADEOUT_DONE\n");
#endif

	// Now process until we get WM_FADEOUT_DONE!
	while (WinGetMsg(hab, &msg, 0, 0, 0) && (msg.msg!=WM_FADEOUT_DONE))
	  WinDispatchMsg(hab, &msg);

#ifdef DEBUG_LOGGING
        AddLog("[fnSaverThread] : WM_FADEOUT_DONE is here, stopping real saving!\n");
#endif
#endif
