...
#define SAVERWINDOW_CLASS "ECSBALL_SCREENSAVER_CLASS"
...
static FILE *internal_OpenNLSFile(char *pchHomeDirectory)
{
  ...
  if (pchFileName)
  {
	...
	sprintf(pchFileName, "%sModules\\eCSBall\\%s.msg", pchHomeDirectory, achRealLocaleName);
#ifdef DEBUG_LOGGING
	...
	do {
	  // Assemble NLS file name
	  sprintf(pchFileName, "%sModules\\eCSBall\\%s.msg", pchHomeDirectory, pchLang);
#ifdef DEBUG_LOGGING
	  AddLog("[internal_OpenNLSFile] : Trying to open NLS file: [");
	  AddLog(pchFileName);
