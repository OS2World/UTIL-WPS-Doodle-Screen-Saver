...
#define ANIMATION_TIMER_TIMEOUT_MSEC 64
HDC     hdcImage;
HPS     hpsImage;
HBITMAP hbmImage;
...
void internal_MakeRandomImagePosition(HWND hwnd)
{
  ...
  // Random start speed
  ptlImageDestSpeed.x = rand() * 10 / RAND_MAX + 1;
  ptlImageDestSpeed.y = rand() * 10 / RAND_MAX + 1;
  if (rand()>RAND_MAX/2)
...
void internal_MakeNewImagePosition(HWND hwnd)
{
  int iScreenWidth = (int) WinQuerySysValue(HWND_DESKTOP, SV_CXSCREEN);
  int iScreenHeight = (int) WinQuerySysValue(HWND_DESKTOP, SV_CYSCREEN);
  BITMAPINFOHEADER2 bmpinfo;
...
//      AddLog("WM_Destroy done\n");
	...
	if (!bImagePositionValid)
	  internal_MakeNewImagePosition(hwnd);
...
static FILE *internal_OpenNLSFile(char *pchHomeDirectory)
{
  ...
  if (pchFileName)
  {
	...
	sprintf(pchFileName, "%sModules\\Padlock\\%s.msg", pchHomeDirectory, achRealLocaleName);
#ifdef DEBUG_LOGGING
	...
	do {
	  // Assemble NLS file name
	  sprintf(pchFileName, "%sModules\\Padlock\\%s.msg", pchHomeDirectory, pchLang);
#ifdef DEBUG_LOGGING
	  AddLog("[internal_OpenNLSFile] : Trying to open NLS file: [");
	  AddLog(pchFileName);
