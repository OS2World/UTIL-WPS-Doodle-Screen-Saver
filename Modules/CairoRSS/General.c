...
static void internal_LoadConfig(char *pchHomeDirectory)
{
  ...
  if (pchHomeEnvVar)
  {
    ...
    strncat(achFileName, ".dssaver\\CairoRSS.CFG", sizeof(achFileName));
  ...
  if (!hFile)
  {
    ...
    snprintf(achFileName, sizeof(achFileName), "%sCairoRSS.CFG", pchHomeDirectory);
    hFile = fopen(achFileName, "rt");
    if (!hFile)
      return; // Could not open file!
  }

  fgets(achRSSURL, sizeof(achRSSURL), hFile);
  while ((strlen(achRSSURL)>0) &&
         ((achRSSURL[strlen(achRSSURL)-1]=='\n') || (achRSSURL[strlen(achRSSURL)-1]=='\r')))
    achRSSURL[strlen(achRSSURL)-1] = 0;
