...
#define SAVERWINDOW_CLASS "SHOWIMG_SCREENSAVER_CLASS"
...
MRESULT EXPENTRY fnSaverWindowProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
  ...
  HWND hwndDlg;
  int rc;
  static char fn_src[CCHMAXPATH+1] = { 0 };
  static GBM gbm;
  static GBMRGB gbmrgb [0x100];
  static BYTE *pbData;
  static HBITMAP hbmBmp;
  static int imageOK;
  static BOOL stretchSmall;
  ...
  switch( msg )
  {
    ...
    case WM_CREATE:
      ...
      imageOK = 0;
      showImage = TRUE;
      strcpy(fn_src , CfgDlgInit.ModuleConfig.sFileName);
      if (LoadBitmap(HWND_DESKTOP, fn_src, "", &gbm, gbmrgb, &pbData)) {
        ...
        if (imageOK == 0) {
          ...
          #ifdef DEBUG_LOGGING
          AddLog("===WM_CREATE=== Cannot convert image to bitmap!\n");
          #endif
        } else {
          if (pbData) free(pbData);
          #ifdef DEBUG_LOGGING
          AddLog("===WM_CREATE=== Image loaded and OK!\n");
          #endif
        }
      } else {
        #ifdef DEBUG_LOGGING
        AddLog("===WM_CREATE=== Cannot open image!\n");
        #endif
      }
      stretchLarge = TRUE;
      stretchSmall = FALSE;
    ...
    case WM_PAINT:
      {
        ...
        if (strcmp(CfgDlgInit.ModuleConfig.sFileName, "") == 0 ) {
            WinDrawText(hps, strlen("Please select an image."), "Please select an image.", &rclRect, SYSCLR_WINDOWTEXT, SYSCLR_WINDOW, DT_ERASERECT | DT_CENTER | DT_VCENTER);
            WinEndPaint(hps);
...
static FILE *internal_OpenNLSFile(char *pchHomeDirectory)
{
  ...
  if (pchFileName)
  {
	...
	sprintf(pchFileName, "%sModules\\ShowImg\\%s.msg", pchHomeDirectory, achRealLocaleName);
#ifdef DEBUG_LOGGING
	...
	do {
	  // Assemble NLS file name
	  sprintf(pchFileName, "%sModules\\ShowImg\\%s.msg", pchHomeDirectory, pchLang);
#ifdef DEBUG_LOGGING
	  AddLog("[internal_OpenNLSFile] : Trying to open NLS file: [");
	  AddLog(pchFileName);
...
SSMODULEDECLSPEC int SSMODULECALL SSModule_Configure(HWND hwndOwner, char *pchHomeDirectory)
{
  ...
  if (WinProcessDlg(hwndDlg) == PB_CONFOK) {
    ...
    if (pchHomeEnvVar) {
      ...
      strncat(chFileName, ".dssaver\\ShowImg.CFG", sizeof(chFileName));
    ...
    if (!hFile) {
      ...
      memset(chFileName, 0 , sizeof(chFileName));
      snprintf(chFileName, sizeof(chFileName), "%sShowImg.CFG", pchHomeDirectory);
      hFile = fopen(chFileName, "wt");
    }
    ...
    if (!hFile) {
      ...
      if (strcmp(CfgDlgInit.ModuleConfig.sFileName, "") == 0)
        sprintf(buffer, "%ssFileName = %s\n", buffer, " ");
      else
        sprintf(buffer, "%ssFileName = %s\n", buffer, CfgDlgInit.ModuleConfig.sFileName);
...
MRESULT EXPENTRY fnConfigDialogProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
  ...
  switch (msg)
  {
    case WM_INITDLG:
     ...
     WinSetWindowText(WinWindowFromID(hwnd, EF_FILENAME), pCfgDlgInit->ModuleConfig.sFileName);
     strcpy (pCfgDlgInit->ModuleConfig.sFileNameOrg, pCfgDlgInit->ModuleConfig.sFileName);
        ...
        switch (SHORT1FROMMP(mp1)) {
          case PB_CONFOK:
            ...
            if (pCfgDlgInit) {
              ...
              WinQueryWindowText(WinWindowFromID(hwnd, EF_FILENAME), 248, pCfgDlgInit->ModuleConfig.sFileName);
              buffer = calloc(10,1);
          ...
          case PB_CONFCANCEL:
            ...
            pCfgDlgInit = (ConfigDlgInitParams_p) WinQueryWindowULong(hwnd, QWL_USER);
            if (pCfgDlgInit) {
               strcpy (pCfgDlgInit->ModuleConfig.sFileName, pCfgDlgInit->ModuleConfig.sFileNameOrg);
            }
          break;
          ...
          case PB_BROWSE: {
            ...
            FileDlg.pszTitle = "Choose an Image File";
            ...
            if (strrchr(temp, '\\') != NULL) {
              ...
              if (!rc) {
                ...
                strncpy(directory, temp,strrchr(temp, '\\') - temp);
                FileDlg.pszIDrive = directory;
              }
            }
            strcpy(FileDlg.szFullFile,"*.bmp;*.jpg;*.jpeg;*.png;*.gif;*.tif;*.tiff;*.tga;*.pcx");

            hwndDlg = WinFileDlg(HWND_DESKTOP, hwnd, &FileDlg);
            if ((hwndDlg!= NULLHANDLE) && (FileDlg.lReturn == DID_OK)) {
              /* copy the user selection to the entry field */
              WinSetWindowText(WinWindowFromID(hwnd, EF_FILENAME), FileDlg.szFullFile);
              if ((bRunning) && (hwndSaverWindow)) {
                 ...
                 if (pCfgDlgInit) {
                   WinQueryWindowText(WinWindowFromID(hwnd, EF_FILENAME), 248, pCfgDlgInit->ModuleConfig.sFileName);
                    ...
                    AddLog("\n");
                 #endif
                 //  WinPostMsg(hwndSaverWindow, WM_IMAGECHANGED, (MPARAM) NULL, (MPARAM) NULL);
                 }
              }
            }
            WinDestroyWindow(hwndDlg);
...
void readConfig (char *pchHomeDirectory) {
  ...
  CfgDlgInit.ModuleConfig.lBlankTime = 60000;
  memset(CfgDlgInit.ModuleConfig.sFileName,0,sizeof(CfgDlgInit.ModuleConfig.sFileName));
  sprintf(CfgDlgInit.ModuleConfig.sFileName, "");
      ...
      while (lineend != NULL) {
        ...
        if (strcmp(term, "iZoomIfSmallerThanScreen\0") == 0)
           if (strcmp(value, "0\0") == 0)
             ...
             CfgDlgInit.ModuleConfig.iZoomIfSmallerThanScreen = 1;
        /*  sFileName  */
        if (strcmp(term, "sFileName\0") == 0)
           if (strcmp(value, " \0") == 0)
             sprintf(CfgDlgInit.ModuleConfig.sFileName, "");
           else
             strcpy(CfgDlgInit.ModuleConfig.sFileName, value);
        /*  lBlankTime  */
        if (strcmp(term, "lBlankTime\0") == 0)
           CfgDlgInit.ModuleConfig.lBlankTime = atol(value);
           if (CfgDlgInit.ModuleConfig.lBlankTime == 0) CfgDlgInit.ModuleConfig.lBlankTime = 60000;
...
static void internal_AdjustConfigDialogControls(HWND hwnd) {
	...
	WinSetWindowPos(hwnd, HWND_TOP, 0, 0, iCX+iCX1+iCX2, 6*(iCY+CYDLGFRAME*4)+WinQuerySysValue(HWND_DESKTOP, SV_CYTITLEBAR), SWP_SIZE);
	...
	// Set the spinbutton and the text
